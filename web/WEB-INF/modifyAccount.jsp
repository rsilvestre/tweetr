			...
			<div id=frame class="span8">
				<form method="post" action="ModifyAccountInfo">
					<fieldset>
						...
						<input type="text" id="userName" name="userName" value="<c:out value="${sessionScope.userSession.userName}"/>" size="20" maxlength="20" required /> 
						<span class="erreur">${form.erreurs['userName']}</span>
							
							
						<label for="firstName">FirstName </label> 
						<input type="text" id="firstName" name="firstName" value="<c:out value="${sessionScope.userSession.firstName}"/>" size="20" maxlength="20" required /> 
						<span class="erreur">${form.erreurs['firstName']}</span>
						
						<label for="lastName">LastName</label> 
						<input type="text" id="lastName" name="lastName" value="<c:out value="${sessionScope.userSession.lastName}"/>" size="20" maxlength="20" required /> 
						<span class="erreur">${form.erreurs['lastName']}</span>
						
						<label for="email">Email</label> 
						<input type="email" id="email" name="email"	value="<c:out value="${sessionScope.userSession.email}"/>" size="60" maxlength="60" required /> 
